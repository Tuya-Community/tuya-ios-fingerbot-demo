...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.home getHomeWeatherSketchWithSuccess:^(TuyaSmartWeatherSketchModel *weatherModel) {
        ...
        [Alert showBasicAlertOnVC:[UIApplication sharedApplication].keyWindow.rootViewController withTitle:@"Failed to get the weather information." message:error.localizedDescription];
    }];
}
...
- (IBAction)dismiss:(UIButton *)sender {
    ...
    UIAlertController *alertViewController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Delete Home", @"") message:NSLocalizedString(@"You are deleting the home.", @"") preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *action = [UIAlertAction actionWithTitle:NSLocalizedString(@"Delete", @"Dismiss a home.") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        [self.home dismissHomeWithSuccess:^{
            [Home setCurrentHome:[TuyaSmartHomeModel new]];
            [self.navigationController popViewControllerAnimated:YES];
        } failure:^(NSError *error) {
            [Alert showBasicAlertOnVC:[UIApplication sharedApplication].keyWindow.rootViewController withTitle:@"Failed to delete the home." message:error.localizedDescription];
        }];
    }];
